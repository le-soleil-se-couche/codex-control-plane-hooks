...
For command tools matched as `Bash`, `exec_command`, or nested `*__exec_command`, only the bound transaction runner is rewritten to `sandbox_permissions=require_escalated`. The rewrite keeps whichever one of `cmd` or `command` the host supplied, preserves `workdir`, `shell`, `login`, `tty`, `yield_time_ms`, `max_output_tokens`, and a nonempty `justification`, and supplies an explicit runner approval question when justification is absent or blank. The original operation keeps its own execution-options digest while the rewritten runner receives a separate digest. Command and options digests are checked as an exact pair at `PreToolUse`, `PermissionRequest`, and `PostToolUse`; sandbox downgrade and shell, working-directory, TTY, or login drift revoke the runner transaction instead of consuming its receipt. Commands outside the bound-runner path keep their original sandbox request.
