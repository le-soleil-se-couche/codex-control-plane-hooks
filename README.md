...
Version `0.2.6` binds each approved push destination and source commit into a one-time ticket, executes the network child from an isolated bare repository, hardens reservation identity, and applies one shared deadline to PATH-only Windows Python discovery and process-tree cleanup. Review the updated manifest and accept the new Hook hash before relying on the broader transaction lifecycle.
...
|---|---|---|---|---|---|
| 0.145.0-alpha.18 bundled desktop CLI | macOS arm64 | 3.9.6 | 218 local tests + manifest/release/plugin checks passed | clean-profile checkout install, Hook discovery/trust, safe allow, dangerous deny, and cross-turn publication transaction resume passed | 2026-07-21 |
...
- Scoped Git/GitHub transactions and the constrained GitHub HTTPS clone lane remain experimental, separately opt-in, and default to disabled. One authorization capsule may list exact `clone`, checkout mutation, `add`, `commit`, and `push` commands plus restrictive safety text. Exact transaction commands bind command digests to their repository scope, branch, target, and the unique canonical `origin` push-URL identity; a push ticket additionally binds the resolved local branch, commit OID, object format, and object database. `PreToolUse` reserves one in-flight operation and rewrites it to a one-time private runner, matching `PermissionRequest` revalidates the runner binding, and matching `PostToolUse` consumes only an atomically matched receipt. The push child uses an isolated bare repository, an immutable OID refspec, a frozen system/global credential-and-HTTP config snapshot with URL rewrites removed, and no workspace-local config or hooks. A successful remote push is consumed even if later local upstream restoration fails; other nonzero, missing, expired, replayed, or mismatched receipts revoke the transaction and matching runner records. The transaction otherwise expires after 30 minutes. Per-repository exact operations retire independently, including heterogeneous multi-repository transactions. A single-scope existing repository may infer its target only from one safe canonical `origin`. Helper, local, insecure, bulk, recursive, multi-ref, custom receive-pack, ambiguous multi-scope, multi-target, force, history-rewrite, and remote-drift forms fail closed. A preauthorized full GitHub HTTPS clone must still use a direct trusted Git command, a new absolute workspace destination, an exact command hash, and post-clone provenance tracking before any declared checkout mutation can run.
...
- Native Windows uses `commandWindows` through a PowerShell 5.1/7 launcher that disables Python Manager automatic installation, resolves `py.exe` and `python.exe` only from explicit `PATH` entries, gives each compatibility probe a bounded ceiling, accepts only exact zero, and preserves the selected Hook process exit code. The `.cmd` file remains a compatibility shim. Windows requires an absolute host-provided `PLUGIN_DATA`, rejects external `CONTROL_PLANE_POLICY`, and relies on the host directory's inherited NTFS DACL. The Hook rejects observed symlinks and reparse points but does not independently audit every DACL ACE.
